...
/*第一个相机的内外参数*/
double f1 = 0.972222208; // focal length 1
/*第二个相机的内外参数*/
double f2 = 0.972222208; // focal length 2 
...
bool  is_correct_pose (math::Matrix3d const &R1, math::Vec3d const & t1
    ...
    // make two depth estimation to be larger than zero
                   ...
                   , math::Vec3d & t)
{
    ...
    // second type of W, means W.transpose
    math::Matrix<double, 3, 3> Wt(0.0);
    Wt(0, 1) = 1.0; Wt(1, 0) = -1.0; Wt(2, 2) = 1.0;

    // use SVD to decompose the essential matrix
    math::Matrix<double, 3, 3> U, S, V;
    math::matrix_svd(E, &U, &S, &V);

    // 保证旋转矩阵 det(R) = 1 (instead of -1). why we set this?
    ...
    /* 相机的姿态一共有4中情况*/
    V.transpose();
    // four different kinds of situations
...
int main(int argc, char* argv[]){
    ...
    // groundtruth of fundamental matrix
    FundamentalMatrix F;
