...
FundamentalMatrix fundamental_8_point (math::Matrix<double, 3, 8> const& points1
    ...
    for (int i=0;i<A.rows;i++){
        ...
        // fill the point information in matrix
        A(i, 0) = p1[0]*p2[0];
        A(i, 1) = p1[1]*p2[0];
        A(i, 2) = p2[0];
        A(i, 3) = p1[0]*p2[1];
    ...
    // solve with SVD method
    math::Matrix<double, 9, 9> V;
    math::matrix_svd<double, 8, 9>(A, nullptr, nullptr, &V);
    math::Vector<double, 9> f = V.col(8);

    // generate F without sigularity
    int k = 0;
    ...
    // regenerate F 
    math::Matrix3d U, S, VV;
    math::matrix_svd<double, 3, 3>(F, &U, &S, &VV); // use pointer not reference

    math::Vec3d diag_S = math::matrix_get_diagonal(S);
    diag_S[2] = 0.0;

    F = U*math::matrix_from_diagonal(diag_S)*VV.transpose();
