...
public:
    ...
    math::Vec2d projection(math::Vec3d const & p3d){
         ...
         *
         */
        // recover a rotation matrix
        math::Matrix3d R;
        math::Vec3d t(t_[0], t_[1], t_[2]);
        int k = 0;
        ...
        // get the 3D point in camera coordinate
        math::Vec3d p3d_cam = R.mult(p3d) + t;

        // remove depth information
        math::Vec2d p(p3d_cam[0]/p3d_cam[2], p3d_cam[1]/p3d_cam[2]);

        // distortion ratio
        double r_2 = p.square_norm();
        double distort_ratio = 1 + dist_[0] * r_2 + dist_[1] * r_2 * r_2;

        math::Vec2d c(c_[0], c_[1]);
        p = f_*distort_ratio*p + c;
    ...
    math::Vec3d pos_in_world(){
        ...
        math::Vec3d pos;
        // the origin of world frame should be negative translation vector
    ...
    math::Vec3d dir_in_world(){
        ...
        // select the final column of the transposed rotation matrix 
...
public:
     ...
     * [ R_[6], R_[7], R_[8] ]
     */
    double R_[9]; // from world to camera

    // 平移向量
    double t_[3]; // in camera frame
};
