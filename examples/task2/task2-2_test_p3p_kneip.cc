...
void pose_p3p_kneip (
    ...
    // all three points should not be in one line
    ...
    {
        math::Vec3d e1 = f1; // f1
        math::Vec3d e3 = f1.cross(f2).normalized(); // f1 x f2
        math::Vec3d e2 = e3.cross(e1);
        // construct the transformation matrix of T
        std::copy(e1.begin(), e1.end(), T.begin() + 0);
        std::copy(e2.begin(), e2.end(), T.begin() + 3);
        std::copy(e3.begin(), e3.end(), T.begin() + 6);

        // transform the f3 into new T transformation matrix 
        f3 = T * f3;
    }

    /* Change camera frame and point order if f3[2] > 0. */
    // make the z coordinate of f3 not positive 
    if (f3[2] > 0.0)
    {   
        // exchange the vector f1 and f2, p1 and p2
        std::swap(p1, p2);
        std::swap(f1, f2);
    ...
    double cos_beta = f1.dot(f2); // angle between f1 and f2  
    double b = 1.0 / (1.0 - MATH_POW2(cos_beta)) - 1; // cot beta
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        // equal to compute the inverse of homogenous transformation matrix
        R = R.transposed();
        C = -R * C;

        solutions->at(i) = R.hstack(C); // 3 * 4
    }
}
...
void compute_p3p (sfm::Correspondences2D3D const& corresp,
    ...
    // distribute three correspondence into three variables
